...
if uploaded_file is not None:
    ...
    # Verifica se a coluna Segmentação existe antes de criar o gráfico
    if "Segmentação" in df.columns:
        df["Segmentação"] = df["Segmentação"].fillna("Desconhecido")
    else:
        df["Segmentação"] = "Desconhecido"
    ...
    selected_column = st.selectbox("Selecione o KPI para Análise", column_options)
    ...
    fig_monthly = px.bar(
        ...
        x="Day",
        y=selected_column,
        title=f"{selected_column} Diário em {selected_month}",
        labels={"Day": "Dia do Mês", selected_column: selected_column},
    )
